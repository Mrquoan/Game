...
$Authorization = $JDecode["KEY"] == "DowrDnASKiBuduejsnfnfPg3J6H8t6xnPVrR"? true : false;
$content = json_decode(file_get_contents($FileName),true);
$Data = "";
$isAdmin = $JDecode["Secret"] =="DowrDnASKiMAgiCYHjrieieudufsMiwHHwbnV27Jvs4" ? true : false;
...
if($isAdmin){
	...
	$Data = $Data.Dec($key)."||".Dec($content[$key]["password"])."||".Dec($content[$key]["ExpireData"])."||".Dec($content[$key]["Actived"])."||".Dec($content[$key]["LastLogin"])."||".Dec($content[$key]["Owner"])."||".Dec($content[$key]["Created"])."||".$IsDev."||".Dec($content[$key]["userAgent"]).'\n';
	}
	else{
